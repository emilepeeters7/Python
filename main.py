...
def optie2(db):
    ...
    try:
        ...
        print("Fout: VIN-nummer bestaat al.")
...
def optie4(db):
    ...
    cursor = db.execute(
        "SELECT id, merk, model FROM brommers WHERE id=?", (brommer_id,)
    )
    brommer = cursor.fetchone()
    ...
    bevestiging = input(
        f"Weet je zeker dat je {brommer[1]} {brommer[2]} wilt verwijderen? (j/n): "
    )
...
def optie6(db):
    ...
    with open("overzicht_brommers.csv", "w", newline="", encoding="utf-8") as file:
        writer = csv.writer(file, delimiter=';')

        writer.writerow([
            "ID", "Merk", "Model", "Productiedatum", "Prijs", "VIN-nummer"
        ])
    ...
    print("Overzicht succesvol opgeslagen in 'overzicht_brommers.csv'")
